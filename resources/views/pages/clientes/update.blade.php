...
@section('content')
    ...
    <form class="form" method="POST" action="{{ route('cliente.atualizar', $findCliente->id) }}">
        @csrf
        @method('PUT') 
        ...
        <div class="mb-3">
            ...
            <input id="localidade" value="{{ isset($findCliente->endereco) ? $findCliente->endereco : old('endereco') }}" class="form-control @error('endereco') is-invalid @enderror" name="endereco">
            @if ($errors->has('endereco'))
                <div class="invalid-feedback">{{ $errors->first('endereco') }}</div>
            @endif
        </div>
        <button type="submit" class="btn btn-success btn-sm">Gravar</button>
        <a class="btn btn-outline-secondary btn-sm" type="button" href="{{ route('clientes.index') }}">Cancelar</a>
    </form>
